...
class Module extends ET_Builder_Module
{
    ...
    public function render($attrs, $content = null, $render_slug)
    {
        $attributes = [
            ...
            'show_company_name' => isset($attrs['company']) ? filter_var(
                $attrs['company'],
            ...
            'show_comments' => isset($attrs['comments']) ? filter_var(
                $attrs['comments'],
                FILTER_VALIDATE_BOOLEAN
            ) : true,
            'comment_length' => isset($attrs['commentlength']) ? (int)$attrs['commentlength'] : 140,
            'only_comments' => isset($attrs['withcomments']) ? filter_var(
                $attrs['withcomments'],
                FILTER_VALIDATE_BOOLEAN
            ) : false,
            'readmore_text' => isset($attrs['readtext']) ? $attrs['readtext'] : esc_html__(
                'Read More Text',
                'give-divi'
            ),
            'loadmore_text' => isset($attrs['loadtext']) ? $attrs['loadtext'] : esc_html__(
                'Load More Text',
                'give-divi'
            ),
